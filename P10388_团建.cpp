...
int main(){
    ...
    for(int i=1;i<=n;i++)  sort(e1[i].begin(),e1[i].end(),cmp1);  //将每个节点的邻接点按权值升序排序
    ...
    while(!q.empty()){            //开始BFS遍历
        ...
        while(i<e1[id1].size()&&j<e2[id2].size()){  //同时遍历两个邻接点列表,直到其中一个遍历完毕
            int w1=a[e1[id1][i]],w2=b[e2[id2][j]];
            if(w1<w2)  i++;       //第一棵树id1的邻接点权值更小,i后移找更大的值
            else if(w1>w2)  j++;  //第二棵树id2的邻接点权值更小,j后移找更大的值
            else if(w1==w2){      //找到权值相同的节点,构成公共前缀的下一个节点
                if(!v1[e1[id1][i]]&&!v2[e2[id2][j]]){         //前提:两个子节点都未被访问过(避免重复)
                    q.push({e1[id1][i],e2[id2][j],t.len+1});  //入队:公共前缀长度+1,后面继续遍历下一层
                }
